      ...
      {/* <button type="button" onClick={() => setOpen(true)}>Open Lightbox</button> */}
        ...
        <div className="flex flex-col items-center" style={{ color: 'green', alignItems: "center" }}> 
         ...
         {/*  <p>Vælg kategori(er):</p> */}
          {CATS.map((cat) => (
            <label key={cat} style={{ display: 'flex-inline', justifyContent: "center", padding: '0px', borderLeft: '0px', letterSpacing: '0px', fontWeight: cat === "Alle" ? "bold" : "unset"}}>
              <input type="radio" checked={selectedCats.includes(cat)} style={{display: 'flex-inline', marginRight: '0px'}} onChange={() => handleToggle(cat)}/>
              {cat}
            </label>
          ))}
         </div> 
